/**
 * StudentPreferenceController — Sprint 18 W4.
 *
 * Exposes `/api/v1/students/me/preferences` GET/PATCH. Mirrors
 * `ParentPreferenceController`. Delegates to `StudentPreferenceService`
 * which gates on the `student_portal` feature flag and the caller's
 * `StudentUser` row.
 */
import {
  Body,
...
import { StudentPermissions } from '../student.constants';
import { StudentPreferenceService } from './student-preference.service';

export class StudentMePreferenceUpdateDto {
...
export class StudentMePreferenceResponseDto {
...
@ApiTags('Student Portal — Me')
@ApiBearerAuth()
@Controller({ path: 'students', version: '1' })
export class StudentPreferenceController {
  constructor(private readonly service: StudentPreferenceService) {}

  @Get('me/preferences')
  @RequirePermissions(StudentPermissions.READ_SELF)
  @ApiOperation({
    summary:
      'Get the calling student\u2019s notification preferences. Lazy-creates defaults on first read.',
  })
  @ApiOkResponse({ type: StudentMePreferenceResponseDto })
  @ApiForbiddenResponse({ description: 'Not a student user or student_portal disabled.' })
  public async getMine(): Promise<StudentMePreferenceResponseDto> {
  ...
  @ApiOperation({
    summary:
      'Patch the calling student\u2019s notification preferences (channels, opt-outs, quiet hours, locale, emergency override).',
  })
  @ApiOkResponse({ type: StudentMePreferenceResponseDto })
  @ApiForbiddenResponse({ description: 'Not a student user or student_portal disabled.' })
  ...
  public async updateMine(
    ...
    @Body() body: StudentMePreferenceUpdateDto,
  ): Promise<StudentMePreferenceResponseDto> {
...
): StudentMePreferenceResponseDto {
  return {
    id: row.id,
